...
// ⭐ PRIMARY API KEY - Your TickerAll API Key
...
async function fetchRealBalance(accountId) {
    try {
        if (!ticker) {
            ...
            return { balance: 0, currency: 'USD', equity: 0, margin: 0, freeMargin: 0, error: 'TickerAll not initialized' };
        }
        if (!accountId) {
            console.warn('⚠️ No account ID');
            return { balance: 0, currency: 'USD', equity: 0, margin: 0, freeMargin: 0, error: 'No account ID' };
        }

        console.log(`🔍 Fetching REAL balance for session: ${accountId}`);
        ...
        if (!accountInfo) {
            return { balance: 0, currency: 'USD', equity: 0, margin: 0, freeMargin: 0, error: 'No account info' };
        }
            ...
            for (const [key, value] of Object.entries(accountInfo)) {
                if (typeof value === 'number' && value > 0 && value < 1000000) {
                    ...
                    if (keyLower.includes('balance') || keyLower.includes('equity') || 
                        ...
                        keyLower.includes('profit') || keyLower.includes('net') ||
                        keyLower.includes('value') || keyLower.includes('money') ||
                        keyLower.includes('asset') || keyLower.includes('capital') ||
                        keyLower.includes('avail') || keyLower.includes('usable')) {
                        balance = value;
        ...
        // Get equity
        if (accountInfo.equity !== undefined && accountInfo.equity !== null) {
        ...
        // Get margin
        if (accountInfo.margin !== undefined && accountInfo.margin !== null) {
            margin = parseFloat(accountInfo.margin) || 0;
        } else if (accountInfo.Margin !== undefined && accountInfo.Margin !== null) {
            margin = parseFloat(accountInfo.Margin) || 0;
        }

        // Get free margin
        if (accountInfo.freeMargin !== undefined && accountInfo.freeMargin !== null) {
            freeMargin = parseFloat(accountInfo.freeMargin) || 0;
        } else if (accountInfo.marginFree !== undefined && accountInfo.marginFree !== null) {
            freeMargin = parseFloat(accountInfo.marginFree) || 0;
        ...
        return { balance: 0, currency: 'USD', equity: 0, margin: 0, freeMargin: 0, error: error.message };
    }
}
...
// ==================== EXNESS CONNECTION (FIXED BALANCE) ====================
app.post('/api/set-exness-creds', authenticate, async (req, res) => {
    try {
        ...
        console.log('📊 Full account info:', JSON.stringify(result.full, null, 2));
...
function detectDivergence(prices, indicator, lookback = 20) {
    ...
    if (priceLows.length >= 2 && indLows.length >= 2) {
        const p1 = priceLows[priceLows.length - 2],
            p2 = priceLows[priceLows.length - 1];
        const i1 = indLows[indLows.length - 2],
            i2 = indLows[indLows.length - 1];
    ...
    if (priceHighs.length >= 2 && indHighs.length >= 2) {
        const p1 = priceHighs[priceHighs.length - 2],
            p2 = priceHighs[priceHighs.length - 1];
        const i1 = indHighs[indHighs.length - 2],
            i2 = indHighs[indHighs.length - 1];
...
// 7. REAL AI Decision (Called EVERY SECOND)
async function getRealAIDecision(symbol, accountId) {
    try {
        ...
        if (rsi < 25) { buyScore += 35;
            reasons.push(`RSI ${rsi.toFixed(1)} (extreme oversold)`); } else if (rsi < 35) { buyScore += 25;
            reasons.push(`RSI ${rsi.toFixed(1)} (oversold)`); } else if (rsi > 75) { sellScore += 35;
            reasons.push(`RSI ${rsi.toFixed(1)} (extreme overbought)`); } else if (rsi > 65) { sellScore += 25;
            reasons.push(`RSI ${rsi.toFixed(1)} (overbought)`); }

        if (trend === 'UP' && longTermTrend === 'UP') { buyScore += 20;
            reasons.push('Strong uptrend'); } else if (trend === 'UP') { buyScore += 10;
            reasons.push('Uptrend'); } else if (trend === 'DOWN' && longTermTrend === 'DOWN') { sellScore += 20;
            reasons.push('Strong downtrend'); } else if (trend === 'DOWN') { sellScore += 10;
            reasons.push('Downtrend'); }

        if (macd.histogram > 0 && macd.macd > macd.signal) { buyScore += 15;
            reasons.push('Bullish MACD'); } else if (macd.histogram < 0 && macd.macd < macd.signal) { sellScore += 15;
            reasons.push('Bearish MACD'); }

        if (bb.lower && currentPrice <= bb.lower * 1.01) { buyScore += 20;
            reasons.push('At lower Bollinger Band'); } else if (bb.upper && currentPrice >= bb.upper * 0.99) { sellScore += 20;
            reasons.push('At upper Bollinger Band'); }

        if (momentum > 0.3) { buyScore += 10;
            reasons.push(`Momentum ${momentum.toFixed(2)}%`); } else if (momentum < -0.3) { sellScore += 10;
            reasons.push(`Momentum ${momentum.toFixed(2)}%`); }

        if (divergence === 'BULLISH') { buyScore += 30;
            reasons.push('Bullish divergence'); } else if (divergence === 'BEARISH') { sellScore += 30;
            reasons.push('Bearish divergence'); }

        if (currentPrice <= fib.level382 * 1.005) { buyScore += 15;
            reasons.push('Fibonacci support'); }
        if (currentPrice >= fib.level618 * 0.995) { sellScore += 15;
            reasons.push('Fibonacci resistance'); }

        if (sentiment > 0.65) { buyScore += 10;
            reasons.push(`Sentiment ${(sentiment * 100).toFixed(0)}%`); } else if (sentiment < 0.35) { sellScore += 10;
            reasons.push(`Sentiment ${(sentiment * 100).toFixed(0)}%`); }

        if (volumeSpike && sentiment > 0.5) { buyScore += 10;
            reasons.push('High volume'); } else if (volumeSpike && sentiment < 0.5) { sellScore += 10;
...
class HalalTradingEngine {
    ...
    async start() {
        console.log(`🕋 Starting REAL AI trading for ${this.userEmail}`);
    ...
    async stop() {
        console.log(`🛑 Stopping REAL AI trading for ${this.userEmail}`);
        this.isActive = false;
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        res.json({
            success: true,
            sessionId,
            message: `✅ REAL AI HALAL TRADING STARTED! Investment: $${investmentAmount} | Target: $${targetProfit} | 🤖 AI analyzes EVERY SECOND!`
        });
    } catch (error) {
